min_assoc.gee = function(target, reps, group, dataset, test, max_k, cvar, wei, selectedVars, pvalues, stats, univariateModels, 
                              selectedVarsOrder, hash, stat_hash, pvalue_hash, correl, se) {
  ...
  results <- list(pvalue = ma_pvalue, stat = ma_stat, pvalues = pvalues, stats = stats, stat_hash=stat_hash, pvalue_hash = pvalue_hash, correl = correl, se = se);
  return(results);
}