waldQPois = function(target, dataset, xIndex, csIndex, wei = NULL, univariateModels = NULL , hash = FALSE, stat_hash = NULL, pvalue_hash = NULL) 
{
  ## initialization
  ...
  } else    fit = glm(target ~., data = as.data.frame( dataset[, c(csIndex, xIndex)] ), family = quasipoisson(link = log), weights = wei)
