normlog.fsreg_2 <- function(target, dataset, iniset, wei = NULL, threshold = 0.05, tol = 2, heavy, robust = FALSE, ncores = 1) {
  ...
  if ( nrow(info) > 1  &  nrow(mat) > 0 ) {
    ...
    }  ## end while ( info[k, 2] < threshold &  k < n - 15  &  tool[ k - 1 ] - tool[ k ] > tol  &  nrow(mat) > 0  )
