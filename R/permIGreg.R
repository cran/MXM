permIGreg = function(target, dataset, xIndex, csIndex, wei = NULL, dataInfo = NULL, univariateModels = NULL, hash = FALSE, stat_hash = NULL, 
  ...
  res <- tryCatch(
    {
      ...
      if ( length(cs) == 0 ) {
        ...
        fit1 = glm(target ~ cs, family = inverse.gaussian(log), weights = wei)
        fit2 = glm(target ~ cs + x, family = inverse.gaussian(log), weights = wei)
        dev2 <- fit2$deviance
    	...
    	if ( stat > 0 ) {
          ...
          while (j <= R & step < thres ) {
            xb <- sample(x, n)  
            bit2 <- glm(target ~ cs + xb, family = inverse.gaussian(link = log), weights = wei)  
            step <- step + ( bit2$deviance < dev2 )
            j <- j + 1
          }
          pvalue <- (step + 1) / (R + 1) 
