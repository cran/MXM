waldZIP = function(target, dataset, xIndex, csIndex, wei = NULL, dataInfo=NULL, univariateModels=NULL , hash = FALSE, stat_hash=NULL, pvalue_hash=NULL,robust=FALSE) 
{
  #initialization
  ...
  res <- tryCatch(
    {
      ...
      if (length(cs) == 0) {
        ...
        fit <- zip.mod(target, x, wei = wei) 
      } else   fit <- zip.mod(target, as.data.frame( dataset[, c(csIndex, xIndex)] ), wei = wei) 
        res = fit$be
        pr = dim(res)[1]
        stat = res[pr, 3]^2
