bic.normlog.fsreg <- function( target, dataset, wei = NULL, tol = 0, heavy = FALSE, robust = FALSE, ncores = 1) {
  ...
  if (ncores <= 1) {
    ...
    if ( !heavy ) {
      ...
      mod <- foreach( i = 1:p, .combine = rbind) %dopar% {
        ww <- glm( target ~ dataset[, i], family =  gaussian(link = log), weights = wei )
        return( BIC( ww ) )
      }
      stopCluster(cl)
    }  else {
    ...
    if ( ncores <= 1 ) {
      ...
      if ( !heavy ) {
        ...
        mod <- foreach( i = 1:pn, .combine = rbind) %dopar% {
          ww <- glm( target ~ dataset[, sel ] + dataset[, mat[i, 1] ], family = gaussian(link = log), weights = wei )
          return( BIC( ww ) )
        }
        stopCluster(cl)
      } else {
