ebic.fbed.mmreg <- function(y, x, univ = NULL, gam = NULL, wei = NULL, K = 0) { 
  dm <- dim(x)
  n <- dm[1]
  p <- dm[2]
  ind <- 1:p
  ...
  if ( (con) < 0 )  con <- 0
  lik1 <- BIC( MASS::rlm(y ~ 1, weights = wei) ) 
  
  runtime <- proc.time()
  
  if ( is.null(univ) ) {
    for ( i in ind ) {
      fit2 <- MASS::rlm( y ~ x[, i], method = "MM", maxit = 2000 ) 
  ...
  s <- which(stat > 0)
    ...
    if ( length(s) > 0 ) {
      ...
      while ( sum(s > 0) > 0 ) {
	    M <- length(sela) + 1
        for ( i in ind[s] )  {
          fit2 <- MASS::rlm( y ~., data = x[, c(sela, i)], method = "MM", maxit = 2000 )
