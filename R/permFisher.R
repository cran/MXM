permFisher = function(target, dataset, xIndex, csIndex, wei = NULL, statistic = FALSE, univariateModels=NULL, hash = FALSE, stat_hash = NULL,
    ...
    res <- tryCatch(
      {
        ...
        if (length(cs) == 0) {
          ...
          #compute the correlation coefficient between x,target directly
           res <- permcor( x, target, R )
           stat <- abs( res[1] )
           pvalue <- log(res[2])
        } else {
          #perform the test with the cs
          er <- .lm.fit(cbind(1, cs), cbind( x, target )  )$residuals 
