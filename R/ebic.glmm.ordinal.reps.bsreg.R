ebic.glmm.ordinal.reps.bsreg <- function(target, dataset, id, reps = NULL, wei = NULL, gam = NULL) {
    ...
    if ( p > n ) {
      ...
      ini <- lme4::glmer( target ~ dataset + reps + (1|id), weights = wei )
      ...
      if ( M == 0 ) {
        mod <- lme4::glmer( target ~ 1 + reps + (1|id), weights = wei)
        bic <- BIC(mod)      
        if (bic0 - bic < 0 ) {
        ...
        ###########   
        for (j in 1:p) {
          mod <- lme4::glmer( target ~ dataset[, -j, drop = FALSE] + reps + (1|id), weights = wei)
        ...
        if ( bic0 - mat[sel, 2] < 0  ) {
          ...
          if ( tool[2] > 0 ) {
            ...
            while ( tool[i - 1] - tool[i ] > 0  &  NCOL(dat) > 0 )  {   
              ...
              ini <- lme4::glmer( target ~ dat + reps + (1|id), weights = wei )
              M <- dim(dat)[2]
              ...
              if ( M == 1 ) {
                ...
                M <- dim(dat)[2] - 1
                for ( j in 1:(M + 1) ) {
                  mod <- lme4::glmer( target ~ dat[, -j, drop = FALSE] + reps + (1|id), weights = wei )
                  bic[j] <- BIC(mod) + con * lchoose(p, M)
                }
                mat[, 2] <- bic
                sel <- which.min( mat[, 2] )
