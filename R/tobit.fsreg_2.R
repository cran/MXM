tobit.fsreg_2 <- function(target, dataset, iniset = NULL, wei = NULL, threshold = 0.05, tol = 2, heavy, robust = FALSE, ncores = 1) {
  ...
  if ( pa == 0 ) {
    ...
    mi<- survival::survreg(target ~., data = as.data.frame( iniset ), weights = wei, dist = "gaussian" )
    ini <- 2* logLik(mi)
  }  
  do <- length(mi$coefficients)
  if (ncores <= 1) {
    for (i in 1:p) {
      mi <- survival::survreg( target ~ . , as.data.frame( dataset[, c(da, pa + i)] ), weights = wei, dist = "gaussian" )
      devi[i] <- 2 * logLik(mi)
    ...
    mod <- foreach( i = 1:p, .combine = rbind, .export = "survival", .packages = "survreg") %dopar% {
      ww <- survival::survreg( target ~., data = as.data.frame( dataset[, c(da, pa + i)] ), weights = wei, dist = "gaussian" )
  ...
  if ( mat[sel, 2] < threshold ) {
    ...
    mi <- survival::survreg( target ~., data = as.data.frame( dataset[, c(da, sel) ] ), weights = wei, dist = "gaussian" )
    la <- logLik(mi)
  ...
  if ( info[k, 2] < threshold  &  nrow(mat) > 0 ) {
    ...
    if ( mat[ina, 2] < threshold ) {
      ma <- survival::survreg( target ~., data=as.data.frame( dataset[, c(da, sela, sel) ] ), weights = wei, dist = "gaussian" )
      la <- logLik(ma)
      tool[k] <-  - 2 * as.numeric( la ) + attr(la, "df") * con
  ...
  if ( d == 0 ) {
    final <- survival::survreg( target ~., data = as.data.frame( iniset ), weights = wei, dist = "gaussian" )
    info <- NULL
    
  } else {
      final <- survival::survreg( target ~., data = as.data.frame( dataset[, c(da, sela) ] ), weights = wei, dist = "gaussian" )
