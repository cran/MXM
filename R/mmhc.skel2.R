mmhc.skel2 <- function(dataset, max_k = 3, threshold = 0.05, test = NULL, rob = FALSE, fast = FALSE, nc = 1, graph = FALSE) {
  ...
  if (nc == 1 || nc < 1 || is.null(nc) ) {
    ...
    } else {   ## not delete edges as you gos
      
      if ( rob == FALSE )  {
        
        pa <- proc.time()
        
        R <- fastR::cora(dataset)
        diag(R) <- 0
        
        up <- sqrt( nu - 3 )   
        if ( test == "testIndFisher" ) {
          stat <- abs( 0.5 * log( (1 + R) / (1 - R) ) * up )  ## absolute of the test statistic
        } else if ( test == "testIndSpearman" ) {
          stat <- abs( 0.5 * log( (1 + R) / (1 - R) ) * up ) / 1.029563  ## absolute of the test statistic
        }
        pval <- log(2) + pt(stat, nu - 3, lower.tail = FALSE, log.p = TRUE)  ## logged p-value    
        flag = numeric(n) + 1
        ...
        for (i in 1:n) {
          ...
          ini$pvalue = pval[i, ]
          ini$flag = flag
          a <- MMPC(i, dataset, max_k = max_k, test = test, ini = ini, threshold = threshold, robust = FALSE)
          sel <- a@selectedVars
          G[i, sel] <- 1 
        }
        runtime <- proc.time() - pa
        ...
        pa <- proc.time()
        for (i in 1:n) {
          a <- MMPC(i, dataset, max_k = max_k, test = test, threshold = threshold, robust = TRUE)
          sel <- a@selectedVars
          G[i, sel] <- 1 
        } 
        runtime <- proc.time() - pa
