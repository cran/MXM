univariateScore = function(target, dataset, test, wei = NULL, dataInfo = NULL, targetID = -1, robust = FALSE, ncores = 1) {
  #how many tests
  ...
  for (i in 1:nTests) {
    ...
    if ( i != targetID ) {
      test_results = test(target, dataset, xIndex=i, csIndex = 0, wei = wei, dataInfo = dataInfo, robust = robust)
