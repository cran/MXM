waldPois = function(target, dataset, xIndex, csIndex, wei = NULL, dataInfo = NULL, univariateModels = NULL , hash = FALSE, stat_hash = NULL, pvalue_hash = NULL,robust = FALSE) 
{
  ## initialization
  ...
  if ( length(cs) != 0 ) {
    if ( is.null(dim(cs)[2]) ) { #cs is a vector
      ...
      if (length(cs) == 0)  {
        fit = glm(target ~ x, poisson, weights = wei)
      } else    fit = glm(target ~., data = as.data.frame( dataset[, c(csIndex, xIndex)] ), poisson, weights = wei)
