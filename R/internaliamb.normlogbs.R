internaliamb.normlogbs <- function(target, dataset, threshold, wei, p, heavy = FALSE) {
  if ( !is.null(dataset) |  p > 0 ) {
    if ( p > 1 ) {
      if ( !heavy ) {
        ini <- glm( target ~.,  data = data.frame(dataset), family = gaussian(link = log), weights = wei, y = FALSE, model = FALSE )
        tab <- drop1( ini, test = "Chisq" )
        dof <- tab[-1, 1]
        stat <- tab[-1, 4]
        
      } else {
        ini <- speedglm::speedglm( target ~.,  data = data.frame(dataset), family = gaussian(link = log), weights = wei )
        dofini <- length( coef(ini) )
        stat <- dof <- numeric(p)
        for (i in 1:p) {
          mod <- speedglm::speedglm( target ~.,  data = data.frame(dataset[, -i]), family = gaussian(link = log), weights = wei )
      ...
      if ( !heavy ) {
        ...
        ini <- speedglm::speedglm( target ~.,  data = data.frame(dataset), family = gaussian(link = log), weights = wei )
        mod0 <- speedglm::speedglm( target ~ 1,  data = data.frame(dataset), family = gaussian(link = log), weights = wei )
      }  
    ...
    if ( length(sel) == 0 ) {
      ...
      if ( p - length(sel) == 0 ) {
        ...
        if ( !heavy ) {
          mod1 <- glm(target ~., data = data.frame(dat), family = gaussian(link = log), weights = wei, y = FALSE, model = FALSE)
          mod0 <- glm(target ~ 1, family = gaussian(link = log), weights = wei, y = FALSE, model = FALSE)
        } else  {
          mod1 <- speedglm::speedglm( target ~ .,  data = data.frame(dat), family = gaussian(link = log), weights = wei )
          mod0 <- speedglm::speedglm( target ~ 1,  data = data.frame(dat), family = gaussian(link = log), weights = wei )
        }  
        ...
        if ( !heavy ) {
          final <- glm(target ~., data = data.frame(dat), family = gaussian(link = log), weights = wei, y = FALSE, model = FALSE)
