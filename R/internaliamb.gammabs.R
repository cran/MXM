  ...
  if ( !is.null(dataset) |  p > 0 ) {
    if ( p > 1 ) {
      ...
      if ( !heavy ) {
        ...
        ini <- speedglm::speedglm( target ~.,  data = dataset, family = Gamma(link = log), weights = wei )
        mod0 <- speedglm::speedglm( target ~ 1,  data = dataset, family = Gamma(link = log), weights = wei )
      }  
    ...
    if ( length(sel) == 0 ) {
      ...
      if ( p - length(sel) == 0 ) {
        ...
        if ( !heavy ) {
          ...
          mod1 <- speedglm::speedglm( target ~ .,  data = dat, family = Gamma(link = log), weights = wei )
          mod0 <- speedglm::speedglm( target ~ 1,  data = dat, family = Gamma(link = log), weights = wei )
        }  
        ...
        } else  final <- speedglm::speedglm( target ~ .,  data = dat, family = Gamma(link = log), weights = wei )
      }
    }
