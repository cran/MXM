ebic.fbed.glmm.cr <- function(y, x, id, univ = NULL, gam = NULL, wei = NULL, K = 0) { 
  ...
  if ( con < 0 )  con <- 0
  lik1 <- BIC( coxme::coxme(y ~ 1 + (1|id), weights = wei) ) 
  
  runtime <- proc.time()
  
  if ( is.null(univ) ) {
    for ( i in ind ) {
      fit2 <- coxme::coxme( y ~ x[, i] + (1|id), weights = wei ) 
  ...
  if ( length(s) > 0 ) {
    ...
    while ( sum(s > 0) > 0 ) {
      M <- length(sela) + 1
      for ( i in ind[s] )  {
        fit2 <- coxme::coxme( y ~ x[, c(sela, i)] + (1|id), weights = wei )
