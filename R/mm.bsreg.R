mm.bsreg <- function(target, dataset, threshold = 0.05, wei = NULL) {
  ...
  if ( p > n ) {
    ...
    if (p == 1) {
      ...
      stat<- 2 * abs(logLik(mod) - logLik(ini) )
      ...
      stat <- dof <- numeric(p)
      for (i in 1:p) {
        mod <- MASS::rlm( target ~.,  data = dataset[, -i ,drop = FALSE], method = "MM", maxit = 2000)
    ...
    if ( info[1, 2] > threshold ) {
        ...
        while ( info[i, 2] > threshold  &  NCOL(dat) > 0 )  {   
          ...
          if ( k == 1 ) {
            ...
            for (j in 1:k) {
              mod <- MASS::rlm( target ~.,  data = dat[, -j, drop = FALSE], method = "MM", maxit = 2000)
  ...
  res <- list(runtime = runtime, info = info, mat = mat, ci_test = "testIndMMReg", final = final ) 
