   ...
   if ( !heavy ) {
    ...
    if ( d >= 1 ) {
      ...
      if ( d == 1 ) {
        ...
        for (i in 1:d) {
          #if ( robust == FALSE ) {
          models[[ i ]] <- glm( y ~., data = as.data.frame( xx[, 1:i] ), weights = wei, family = binomial, y = FALSE, model = FALSE )
          #} else { 
    ...
    ##############
     ...
     if ( d >= 1 ) {
       ...
       if ( d == 1 ) {
         ...
         for (i in 1:d) {
           models[[ i ]] <- speedglm::speedglm( y ~., data = as.data.frame( xx[, 1:i] ), weights = wei, family = binomial, y = FALSE, model = FALSE )
