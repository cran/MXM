...
pois.mxm <- function(train_target, sign_data, sign_test, wei){
    ...
    x = sign_test
    # preds <- predict(sign_model, newdata=data.frame(sign_test), type = 'response')
    predict( sign_model, newdata=data.frame(x), type = 'response' )
...
## binomial regression
 binom.mxm <-  function(train_target, sign_data, sign_test){
   ...
   x = sign_test
   predict( sign_model, newdata = data.frame(x), type = 'response' ) * N1 
...
lmrob.mxm <- function(train_target, sign_data, sign_test, wei){ ## used for univariate and multivariate target in classical regression
    ...
    x = sign_data
    sign_model <- MASS::rlm( train_target ~ ., data = data.frame(x), maxit = 2000, weights = wei );
    x = sign_test
    predict( sign_model, newdata=data.frame(x) )
