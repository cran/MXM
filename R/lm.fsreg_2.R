lm.fsreg_2 <- function(target, dataset, iniset = NULL, threshold = 0.05, stopping = "BIC", tol = 2, robust = FALSE, ncores = 1 ) {
  ...
  if (ncores <= 1) {
    ...
    if ( robust == FALSE ) {  ## Non robust
      for (i in 1:p) {
        ww = lm( target ~., data = as.data.frame( dataset[, c(da, pa + i)] ) )
        tab = anova(ww)
        stat[i] = tab[pa + k, 4] 
      ...
      for (i in 1:p) {
        ...
        # tab = aovlmrob( ww )
        ww = MASS::rlm( target ~., data = as.data.frame( dataset[, c(da, pa + i)] ), maxit = 2000 ) 
    ...
    if ( robust == FALSE ) {  ## Non robust
      ...
      mod <- foreach( i = 1:p, .combine = rbind, .export = c("rlm"), .packages = "MASS" ) %dopar% {
        ...
        ww = rlm( target ~., data = as.data.frame( dataset[, c(da, pa + i)] ), maxit = 2000 ) 
  ...
  if ( mat[sel, 2] < threshold ) {
    ...
    if ( stopping == "adjrsq" ) {
      ...
      if ( robust == FALSE ) {
        ...
        mi = MASS::rlm( target ~., data = as.data.frame( dataset[, c(da, sel) ] ), maxit = 2000)
  ...
  if ( info[k, 2] < threshold  &  nrow(mat) > 0 ) {
    ...
    if ( ncores <= 1 ) {
      if ( robust == FALSE ) {      
        for (i in 1:pn) {
          ww = lm( target ~., data = as.data.frame( dataset[, c(da, sel, mat[pa + i, 1]) ] ) )
          tab = anova( ww )
          mat[i, 3] = tab[pa + k, 4] 
          df1 = tab[pa + k, 1]   ;  df2 = tab[pa + k + 1, 1]
        ...
        for (i in 1:pn) {
          ...
          # mat[i, 3] = tab[3, 2] 
          # mat[i, 2] = tab[3, 3]
          ww = MASS::rlm( target ~., data = as.data.frame( dataset[, c(da, sel, mat[pa + i, 1]) ] ), maxit = 2000 )
      ...
      if ( robust == FALSE ) {  ## Non robust
        ...
        mod <- foreach( i = 1:pn, .combine = rbind, .export = c("rlm"), .packages = "MASS" ) %dopar% {
          ...
          # pval[i] <- tab[3, 3]
          # mata[i, ] <- c(pval[i], stat[i]) 
          ww <- rlm( target ~., data = as.data.frame( dataset[, c(da, sel, mat[pa + i, 1]) ] ), maxit = 2000 )
  ...
  if ( stopping == "adjrsq" ) {
    if ( mat[ina, 2] < threshold ) {
      if ( robust == FALSE ) {
        mi = lm( target ~., data=as.data.frame( dataset[, c(da, sela, sel) ] ) )
        tool[k] <- as.numeric( summary( mi )[[ 9 ]] )
      } else {
        # mi = robust::lmRob( target ~ dataset[, sela] + dataset[, sel], control = cont )
        # r2 <- as.numeric( summary( mi )[[ 8 ]] )               
        mi = MASS::rlm( target ~., data=as.data.frame( dataset[, c(da, sela, sel) ] ), maxit = 2000)
        r2 = cor( target, fitted(mi) )^2
        tool[k] = 1 - (1 - r2) * (n - 1) / ( n - length( coef(mi) ) - 1)
      }
    ...
    if ( mat[ina, 2] < threshold ) {
      if ( robust == FALSE ) {            
        ...
        # mi = robust::lmRob( target ~ dataset[, sela] + dataset[, sel], control = cont )
        mi = MASS::rlm(target ~ target ~., data = as.data.frame( dataset[, c(da, sela, sel) ] ), maxit = 2000)
        tool[k] = BIC(mi)
      }
  ...
  if ( nrow(info) > 1  &  nrow(mat) > 0 ) {
    ...
    while ( ( info[k, 2] < threshold ) &  ( k < n ) & ( abs( tool[ k ] - tool[ k - 1 ] ) > tol ) & ( nrow(mat) > 0 ) )  {
      ...
      if ( ncores <= 1 ) {
        if ( robust == FALSE ) {
          for ( i in 1:pn ) {
            ww <- lm( target ~., data = as.data.frame( dataset[, c(da, sela, mat[pa + i, 1]) ] )  )
            tab <- anova( ww )
            mat[i, 3] <- tab[pa + k, 4] 
            df1 <- tab[pa + k, 1]   ;  df2 = tab[pa + k + 1, 1]
            mat[i, 2] <- pf( mat[i, 3], df1, df2, lower.tail = FALSE, log.p = TRUE )
          ...
          for (i in 1:pn) {
            ...
            # mat[i, 3] = tab[3, 2] 
            # mat[i, 2] = tab[3, 3]
            ww = MASS::rlm( target ~., data = as.data.frame( dataset[, c(da, sela, mat[pa + i, 1]) ] ), maxit = 2000 )
            sta[i] = 2 * as.numeric( logLik(ww) )
            dof[i] = length( coef(ww) )
          } 
          mat[, 3] = abs( sta - do )
        ...
        if ( robust == FALSE ) {  ## Non robust
          ...
          mod <- foreach( i = 1:pn, .combine = rbind, .export = c("rlm"), .packages = "MASS" ) %dopar% {
            ...
            # pval[i] <- tab[k + 1, 3]
            # mata[i, ] <- c( pval[i], stat[i] ) 
            ww <- rlm( target ~., data = as.data.frame( dataset[, c(da, sela, mat[pa + i, 1]) ] ), maxit = 2000 )
            mata[i, ] = c( 2 * as.numeric( logLik(ww) ), length( coef(ww) ) )
          }
          stopCluster(cl)  
          difa = abs( mod[, 1] - do )
      ...
      if ( stopping == "BIC" ) {
        if ( mat[ina, 2] < threshold ) {
          if ( robust == FALSE ) {
            ma = lm( target ~., data = as.data.frame( dataset[, c(da, sela, sel)] ) )
            tool[k] <- BIC( ma )
          } else {
            # ma = robust::lmRob( target ~., data = as.data.frame( dataset[, c(sela, sel)] ), control = cont )
            ma = MASS::rlm( target ~., data = as.data.frame( dataset[, c(da, sela, sel)] ), maxit = 2000 )
            tool[k] =  BIC(ma)
          }
  ...
  if ( d == 0 ) {
    final <- summary( lm( target ~., data = as.data.frame( iniset ) ) )
    info = NULL
  ...
  if ( d >= 1 ) {
    ...
    if ( d == 1 ) {
      ...
      if ( robust == FALSE ) {
        models[[ 1 ]] <- lm( target ~., data = as.data.frame( xx ) )
      } else {
        # models[[ 1]] <- final <- robust::lmRob( target ~., data = as.data.frame( xx ), control = cont )
        models[[ 1 ]] <- MASS::rlm( target ~., data = as.data.frame( xx ), maxit = 2000 )
      }
