glm.fsreg_2 <- function(target, dataset, iniset = NULL, threshold = 0.05, tol = 2, robust = FALSE, ncores = 1) {
  ...
  if (ncores <= 1) {
    #if ( robust == FALSE ) {  ## Non robust
    for (i in 1:p) {
      mi <- glm( target ~ . , as.data.frame( dataset[, c(da, pa + i)] ), family = oiko )
      devi[i] <- mi$deviance
      dof[i] = length( coef( mi ) ) 
    }
    ...
    mata <- matrix(0, p, 2)
    mod <- foreach( i = 1:p, .combine = rbind) %dopar% {
      ww <- glm( target ~., data = as.data.frame( dataset[, c(da, pa + i)] ), family = oiko )
      mata[i, ] <- c( ww$deviance, length( coef( ww ) )  )
    }
  ...
  if ( mat[sel, 2] < threshold ) {
    ...
    #if ( robust == FALSE ) {
    mi <- glm( target ~., data = as.data.frame( dataset[, c(da, sel) ] ), family = oiko )
    tool[k] <- BIC( mi )
    #} else {
    #  mi <- robust::glmRob( target ~ dataset[, sel], family = oiko, maxit = maxit )
    #  tool[1] <- mi$deviance + length( coef( mi ) ) * log(n)
  ...
  if ( info[k, 2] < threshold  &  nrow(mat) > 0 ) {
    ...
    if ( ncores <= 1 ) {
      ...
      #if ( robust == FALSE ) {  ## Non robust
      for ( i in 1:pn ) {
        ww <- glm( target ~., data = as.data.frame( dataset[, c(da, sela, mat[pa + i, 1]) ] ), family = oiko )
        devi[i] <- ww$deviance
        dof[i] = length( coef( ww ) )          
      }
    ...
    if ( mat[ina, 2] < threshold ) {
      #if ( robust == FALSE ) {
      ma <- glm( target ~., data=as.data.frame( dataset[, c(da, sela, sel) ] ), family = oiko )
      tool[k] <- BIC( ma )
      #} else {
      #  ma <- robust::glmRob( target ~  dataset[, sela] + dataset[, sel], family = oiko, maxit = maxit )
      #  tool[k] <- ma$deviance + length( coef( ma ) ) * log(n)
  ...
  if ( nrow(info) > 1  &  nrow(mat) > 0 ) {
    while ( ( info[k, 2] < threshold ) &  ( k < n ) & ( tool[ k - 1 ] - tool[ k ] > tol ) & ( nrow(mat) > 0 ) )  {
      ...
      if (ncores <= 1) {  
        ...
        #if ( robust == FALSE ) {  ## Non robust
        for ( i in 1:pn ) {
          ma <- glm( target ~., data = as.data.frame( dataset[, c(da, sela, mat[pa + i, 1] ) ] ), family = oiko )
          devi[i] <- ma$deviance
          dof[i] = length( coef( ma ) ) 
        }
  ...
  if ( d == 0 ) {
    final <- summary( glm( target ~., data = as.data.frame( iniset ), family = oiko ) )
    info = NULL
