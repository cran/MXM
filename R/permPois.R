permPois = function(target, dataset, xIndex, csIndex, wei = NULL, univariateModels = NULL , hash = FALSE, stat_hash = NULL, 
 pvalue_hash = NULL, threshold = 0.05, R = 999) {
  ...
  if(xIndex < 0 || csIndex < 0) {
    ...
    results <- list(pvalue = pvalue, stat_hash=stat_hash, pvalue_hash=pvalue_hash);
    return(results);
  }
  ...
  if ( length(cs) == 0 )  {
    fit2 = glm(target ~ x, poisson, weights = wei, model = FALSE)
    dev2 <- fit2$deviance  
	...
	if (stat > 0) {
      ...
      while (j <= R & step < thres ) {
        xb <- sample(x, n)  
        bit2 <- glm(target ~ xb, poisson, weights = wei, model = FALSE)  
    ...
    fit1 = glm(target ~ cs, poisson, weights = wei, model = FALSE)
    fit2 = glm(target ~ cs + xb, poisson, weights = wei, model = FALSE)
    dev2 <- fit2$deviance
	...
	if ( stat > 0 ) {
      ...
      while (j <= R & step < thres ) {
        xb <- sample(x, n)  
        bit2 <- glm(target ~ cs + xb, poisson, weights = wei, model = FALSE)  
        step <- step + ( bit2$deviance < dev2 )
        j <- j + 1
      }
      pvalue <- log( (step + 1) / (R + 1) ) 
