permReg = function(target, dataset, xIndex, csIndex, wei = NULL, dataInfo = NULL, univariateModels=NULL , hash = FALSE, stat_hash = NULL, 
  ...
  res <- tryCatch(
{
  ...
  if ( length(cs) == 0 ) {
    if (robust) { ## robust estimation
       ...
       fit1 = MASS::rlm(target ~ 1, weights = wei, method = "MM", maxit = 2000)
       fit2 = MASS::rlm(target ~ x, weights = wei, method = "MM", maxit = 2000)
       lik2 = as.numeric( logLik(fit2) ) 
       stat = 2 * lik2 - 2 * as.numeric( logLik(fit1) )
       step <- 0
       j <- 1		
       n <- length(target)
       while (j <= R & step < thres ) {
         xb <- sample(x, n)  
         bit2 <- MASS::rlm(target ~ xb, weights = wei, method = "MM", maxit = 2000)
     ...
     if (robust) { ##robust estimation
       fit1 = MASS::rlm(target ~ cs, weights = wei, method = "MM", maxit = 2000)
       fit2 = MASS::rlm(target ~ cs + x, weights= wei, method = "MM", maxit = 2000)
       lik2 <- 2 * as.numeric( logLik(fit2) ) 
       stat = lik2 - 2 * as.numeric( logLik(fit1) )
       step <- 0
       j <- 1		
       n <- length(target)
       while (j <= R & step < thres ) {
         xb <- sample(x, n)  
         bit2 <- MASS::rlm(target ~ cs + xb, weights = wei, method = "MM", maxit = 2000 )
         step <- step + ( as.numeric( logLik(bit2) ) > lik2 )
         j <- j + 1
       }
       pvalue <- (step + 1) / (R + 1) 
