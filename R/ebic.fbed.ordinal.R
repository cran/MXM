ebic.fbed.ordinal <- function(y, x, univ = NULL, gam = NULL, wei = NULL, K = 0) { 
  dm <- dim(x)
  n <- dm[1]
  p <- dm[2]
  ind <- 1:p
  ...
  if ( (con) < 0 )  con <- 0
  lik1 <- BIC( ordinal::clm(y ~ 1, weights = wei) ) 
  
  runtime <- proc.time()
  
  if ( is.null(univ) ) {
    for ( i in ind ) {
      fit2 <- ordinal::clm( y ~ x[, i], weights = wei, model = FALSE ) 
  ...
  s <- which(stat > 0)
    ...
    if ( length(s) > 0 ) {
      ...
      while ( sum(s > 0) > 0 ) {
	    M <- length(sela) + 1
        for ( i in ind[s] )  {
          fit2 <- ordinal::clm( y ~., data = x[, c(sela, i)], weights = wei, model = FALSE )
