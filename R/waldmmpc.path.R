waldmmpc.path <- function(target, dataset, wei = NULL, max_ks = NULL, thresholds = NULL, test = NULL, user_test = NULL, robust = FALSE, ncores = 1) {
  ...
  for (i in 1:nalpha) {
    for (j in 1:nmaxk) {
      ...
      results <- wald.mmpc(target, dataset, max_k = max_ks[j], threshold = alphas[i], test, ini = iniset, wei = wei, hash = TRUE, hashObject = inihash, ncores = ncores)
      iniset <- results@univ
  ...
  rownames(bic) = rownames(size) = paste("alpha=", alphas, sep = "")
  colnames(bic) = colnames(size) = paste("max_k=", max_ks, sep = "")
