  ...
  if ( !is.null(dataset) |  p > 0 ) {
    ...
    if ( p > 1 ) {
      if ( !heavy ) {
        ...
        for (i in 1:p) {
          fit2 = speedglm::speedlm( target ~., data = dataset[, -i, drop = FALSE], weights = wei )
    ...
    if ( length(sel) == 0 ) {
      ...
      if ( p - length(sel) == 0 ) {
        ...
        if ( !heavy ) {
          ...
          mod1 <- speedglm::speedlm( target ~ ., data = dat, weights = wei )
          mod0 <- speedglm::speedlm( target ~ 1, data = dat, weights = wei )
        ...
        if ( !heavy ) {
          ...
          } else    final <- MASS::rlm( target ~., data = dataset, weights = wei, maxit = 2000, method = "MM" )
        } else  final <- speedglm::speedlm( target ~ .,  data = dat, weights = wei )
      }
    }
