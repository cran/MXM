quasipois.bsreg <- function(target, dataset, threshold = 0.05, wei = NULL) {
  ...
  if ( p > n ) {
    ...
    ini <- glm( target ~.,  data = dataset, family = quasipoisson(link = log), weights = wei, y = FALSE, model = FALSE )
    ...
    if ( mat[sel, 2] < threshold ) {
      ...
      res <- list(runtime = runtime, info = matrix(0, 0, 3), mat = mat, ci_test = "testIndQPois", final = ini ) 
      ...
      if ( info[1, 2] > threshold & dim(mat)[1]>0 ) {
        ...
        while ( info[i, 2] > threshold  &  dim(dat)[2] > 0 )  {   
          i <- i + 1
          k <- p - i + 1
          ini <- glm( target ~., data = dat, family = quasipoisson(link = log), weights = wei, y = FALSE, model = FALSE )
          dofini <- length(ini$coefficients)
          if ( k == 1 ) {
            mod <- glm(target ~ 1, data = dat, family = quasipoisson(link = log), weights = wei)
        ...
        res <- list(runtime = runtime, info = info, mat = mat, ci_test = "testIndQPois", final = final ) 
        
      } else {
        runtime <- proc.time() - runtime
        res <- list(runtime = runtime, info = info, mat = NULL, ci_test = "testIndQPois", final = mod ) 
      }
    }  
  }  
  res
