ebic.lm.bsreg <- function(target, dataset, gam = NULL, wei = NULL) {
  ...
  if ( p > n ) {
    ...
    if ( M == 0 ) {
      ...
      ###########   
    for (j in 1:p) {
      mod <- lm( target ~.,  data = dataset[, -j, drop = FALSE], weights = wei)
    ...
    if ( tool[2] > 0 ) {
      ...
      while ( tool[i - 1] - tool[i ] > 0  &  NCOL(dat) > 0 )  {   
        ...
        ini <- lm( target ~., data = dat, weights = wei )
	     	M <- dim(dat)[2]
        ...
        if ( M == 1 ) {
          ...
          bic <- numeric(M)
		      M <- dim(dat)[2] - 1
          for ( j in 1:(M + 1) ) {
            mod <- lm( target ~.,  data = dat[, -j, drop = FALSE], weights = wei )
            bic[j] <-  BIC(mod) + con * lchoose(p, M)
          }
          mat[, 2] <- bic
          sel <- which.min( mat[, 2] )
