ebic.wr.bsreg <- function(target, dataset, wei = NULL, gam = NULL) {
  ...
  if ( p > n ) {
    ...
    ini <- survival::survreg( target ~.,  data = dataset, weights = wei, dist = "weibull" )
    ...
    if ( M == 0 ) {
      mod <- survival::survreg( target ~ 1, dist = "weibull" )
      ...
      ########### 
    for (j in 1:p) {
      mod <- survival::survreg( target ~.,  data = dataset[, -j, drop = FALSE], weights = wei, dist = "weibull" )
      bic[j] <-  - 2 * logLik(mod) + (length(mod$coefficients) + 1) * logn + con * lchoose(p, M)
    }
    
    mat <- cbind(1:p, bic )
    sel <- which.min( mat[, 2] )
    info <- matrix( 0, nrow = 1, ncol = 2 )
    colnames(info) <- c("Variables", "eBIC")
    colnames(mat) <- c("Variables", "eBIC")
    ...
    if ( bic0 - mat[sel, 2] < 0  ) {
      ...
      if ( tool[2] > 0 ) {
        ...
        while ( tool[i - 1] - tool[i ] > 0  &  NCOL(dat) > 0 )  {   
          ...
          ini <- survival::survreg( target ~., data = dat, weights = wei, dist = "weibull" )
          M <- dim(dat)[2]
          bic0 <-  - 2 * logLik(ini) + (length(ini$coefficients) + 1) * logn + con * lchoose(p, M)
          i <- i + 1        

          if ( M == 1 ) {
            mod <- survival::survreg(target ~ 1, weights = wei, dist = "weibull" )
            bic <-  - 2 * logLik(mod) + (length(mod$coefficients) + 1) * logn 
            tool[i] <- bic
            if (bic0 - bic < 0 ) {
              runtime <- proc.time() - tic
            ...
            M <- dim(dat)[2] - 1
            for ( j in 1:(M + 1) ) {
              mod <- survival::survreg( target ~.,  data = dat[, -j, drop = FALSE], weights = wei, dist = "weibull" )
              bic[j] <-  - 2 * logLik(mod) + (length(mod$coefficients) + 1) * logn + con * lchoose(p, M)
            }
            mat[, 2] <- bic
            sel <- which.min( mat[, 2] )
