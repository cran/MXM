testIndFisher = function(target, dataset, xIndex, csIndex, dataInfo=NULL, univariateModels=NULL , hash = FALSE, stat_hash=NULL, pvalue_hash=NULL)
{
  ...
  #   DATASET: a numeric data matrix containing the variables for performing the test.
  ...
  res <- tryCatch(
{
  ...
  df = length(x) - ncol(as.matrix(cs)) - 3; #degrees of freedom
  w = sqrt(df)*z;
