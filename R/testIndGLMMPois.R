testIndGLMMPois = function(target, reps = NULL, group, dataset, xIndex, csIndex, wei =  NULL, univariateModels = NULL,
 hash = FALSE, stat_hash = NULL, pvalue_hash = NULL, slopes = FALSE) {
  ...
  if (length(cs) == 0)  {
   ...
   if ( is.null(reps) ) {
      fit2 = lme4::glmer( target ~ (1|group) + dataset[, xIndex], weights = wei, REML = FALSE, family = poisson ) 
   } else {
      reps <- reps 
      if ( slopes ) {
        fit2 = lme4::glmer( target ~ reps + (reps|group) + dataset[, xIndex], weights = wei, REML = FALSE , family = poisson ) 
      } else {
        reps = reps 
        fit2 = lme4::glmer( target ~ reps + (1|group) + dataset[, xIndex], weights = wei, REML = FALSE , family = poisson )  
      }
   }
   
  } else {
    if ( is.null(reps) ) {
      fit2 = lme4::glmer( target ~ (1|group) + dataset[, csIndex] + dataset[, xIndex], weights = wei, REML = FALSE , family = poisson )    
    } else {
      reps = reps 
      if (slopes ) {
