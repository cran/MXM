fbed.glm2 <- function(y, x, alpha = 0.05, univ = NULL, wei = NULL, K = 0, type = "quasibinomial") { 
  if (type == "quasibinomial") {
    oiko <- quasibinomial(link = logit)
  } else if (type == "quasipoisson") {
    oiko <- quasipoisson(link = log)
  } else if (type == "gamma") {
    oiko <- Gamma(log)
  } else if (type == "normlog") {
    oiko <- gaussian(log)
  } 
    
  dm <- dim(x)
  p <- dm[2]
  n <- dm[1]
  ind <- 1:p
  sig <- log(alpha)
  devi1 <- glm(y ~ 1, family = oiko, weights = wei, model = FALSE)$deviance
  devi2 <- numeric(p)
  phi <- numeric(p)
  dof <- numeric(p)
  sela <- NULL
  card <- 0
  sa <- NULL
  pva <- NULL
  
  runtime <- proc.time()
  
  if ( is.null(univ) ) {
    for ( i in ind ) {
      fit2 <- glm( y ~ x[, i], family = oiko, weights = wei )
      devi2[i] <- fit2$deviance
      phi[i] <- summary(fit2)[[ 14 ]]
      dof[i] <- length(fit2$coefficients)
    }
    n.tests <- p
    stat <- (devi1 - devi2)/(dof - 1) / phi
    pval <- pf(stat, dof - 1, n - dof, lower.tail = FALSE, log.p = TRUE)
  } else {
    stat <- univ$stat
    pval <- univ$pvalue
    n.tests <- 0
    sel <- which.min(pval)
    devi2 <- numeric(p)
    devi2[sel] <- glm( y ~ x[, sel], family = oiko, weights = wei )$deviance
  }  
  s <- which(pval < sig)
  
  if ( length(s) > 0 ) {
    sel <- which.min(pval)
    sela <- sel
    s <- s[ - which(s == sel) ]
    devi1 <- devi2[sel] 
    d1 <- dim( model.matrix( y~., data.frame(x[, sel]) ) )[2] - 1
    sa <- stat[sel]
    pva <- pval[sel]
    devi2 <- rep( devi1, p )
    dof <- phi <- rep(10000, p)
    #########
    while ( sum(s>0) > 0 ) {
      for ( i in ind[s] )  {
        fit2 <- glm( y ~., data = x[, c(sela, i)], family = oiko, weights = wei, model = FALSE )
        devi2[i] <- fit2$deviance
        phi[i] <- summary(fit2)[[ 14 ]]
        dof[i] <- length(fit2$coefficients)
      }
      n.tests <- n.tests + length( ind[s] ) 
      stat <- (devi1 - devi2) / (dof - d1) / phi
      pval <- pf(stat, dof - d1, n - dof, lower.tail = FALSE, log.p = TRUE)
      s <- which(pval < sig) 
      sel <- which.min(pval) * ( length(s)>0 )
      sa <- c(sa, stat[sel]) 
      pva <- c(pva, pval[sel])
      sela <- c(sela, sel[sel>0])
      s <- s[ - which(s == sel) ]
      if (sel > 0) {
        devi1 <- devi2[sel] 
        d1 <- dof[sel]
        devi2 <- rep(devi1, p)
        dof <- phi <- rep(10000, p)
      } 
    } ## end while ( sum(s > 0) > 0 )
    
  card <- sum(sela > 0)
  
  if (K == 1) {
    for ( i in ind[-sela] )  {
      fit2 <- glm( y ~., data = x[, c(sela, i)], family = oiko, weights = wei, model = FALSE )
      devi2[i] <- fit2$deviance
      phi[i] <- summary(fit2)[[ 14 ]]
      dof[i] <- length(fit2$coefficients)
    }
    n.tests[2] <- length( ind[-sela] )
    stat <- (devi1 - devi2) / (dof - d1) / phi
    pval <- pf(stat, dof - d1, n - dof, lower.tail = FALSE, log.p = TRUE)
    s <- which(pval < sig)
    sel <- which.min(pval) * ( length(s)>0 )
    sa <- c(sa, stat[sel]) 
    pva <- c(pva, pval[sel])
    sela <- c(sela, sel[sel>0])
    s <- s[ - which(s == sel) ]
    if (sel > 0) {
      devi1 <- devi2[sel] 
      d1 <- dof[sel]
      devi2 <- rep(devi1, p)
      dof <- phi <- rep(10000, p)
    } 
    while ( sum(s>0) > 0 ) {
      for ( i in ind[s] )  {
        fit2 <- glm( y ~., data = x[, c(sela, i)], family = oiko, weights = wei, model = FALSE )
        devi2[i] <- fit2$deviance
        phi[i] <- summary(fit2)[[ 14 ]]
        dof[i] <- length(fit2$coefficients)
      }
      n.tests[2] <- n.tests[2] + length( ind[s] )
      stat <- (devi1 - devi2) / (dof - d1) / phi
      pval <- pf(stat, dof - d1, n - dof, lower.tail = FALSE, log.p = TRUE)
      s <- which(pval < sig)
      sel <- which.min(pval) * ( length(s)>0 )
      sa <- c(sa, stat[sel]) 
      pva <- c(pva, pval[sel])
      sela <- c(sela, sel[sel>0])
      s <- s[ - which(s == sel) ]
      if (sel > 0) {
        devi1 <- devi2[sel] 
        d1 <- dof[sel]
        devi2 <- rep(devi1, p)
        dof <- phi <- rep(10000, p)
      } 
    } ## end while ( sum(s>0) > 0 ) 
    card <- c(card, sum(sela>0) )
  }  ## end if ( K == 1 ) 
  
  if ( K > 1) {
    
    for ( i in ind[-sela] )  {
      fit2 <- glm( y ~., data = x[, c(sela, i)], family = oiko, weights = wei, model = FALSE )
      devi2[i] <- fit2$deviance
      phi[i] <- summary(fit2)[[ 14 ]]
      dof[i] <- length(fit2$coefficients)
    }
    n.tests[2] <- length( ind[-sela] ) 
    stat <- (devi1 - devi2)/(dof - d1) / phi
    pval <- pf(stat, dof - d1, n - dof, lower.tail = FALSE, log.p = TRUE)
    s <- which(pval < sig)
    sel <- which.min(pval) * ( length(s)>0 )
    sa <- c(sa, stat[sel]) 
    pva <- c(pva, pval[sel])
    sela <- c(sela, sel[sel>0])
    s <- s[ - which(s == sel) ]
    if (sel > 0) {
      devi1 <- devi2[sel] 
      d1 <- dof[sel]
      devi2 <- rep(devi1, p)
      dof <- phi <- rep(10000, p)
    } 
    while ( sum(s > 0) > 0 ) {
      for ( i in ind[s] )  {
        fit2 <- glm( y ~., data = x[, c(sela, i)], family = oiko, weights = wei, model = FALSE )
        devi2[i] <- fit2$deviance
        phi[i] <- summary(fit2)[[ 14 ]]
        dof[i] <- length(fit2$coefficients)
      }
      n.tests[2] <- n.tests[2] + length( ind[s] )  
      stat <- (devi1 - devi2) / (dof - d1) / phi
      pval <- pf(stat, dof - d1, n - dof, lower.tail = FALSE, log.p = TRUE)
      s <- which(pval < sig)
      sel <- which.min(pval) * ( length(s)>0 )
      sa <- c(sa, stat[sel]) 
      pva <- c(pva, pval[sel])
      sela <- c(sela, sel[sel>0])
      s <- s[ - which(s == sel) ]
      if (sel > 0) {
        devi1 <- devi2[sel] 
        d1 <- dof[sel]
        devi2 <- rep(devi1, p)
        dof <- phi <- rep(10000, p)
      } 
    } ## end while ( sum(s>0) > 0 ) 
    
    card <- c(card, sum(sela > 0) )
    vim <- 1
    while ( vim < K  & card[vim + 1] - card[vim] > 0 ) {
      vim <- vim + 1
      for ( i in ind[-sela] )  {
        fit2 <- glm( y ~., data = x[, c(sela, i)], family = oiko, weights = wei, model = FALSE )
        devi2[i] <- fit2$deviance
        phi[i] <- summary(fit2)[[ 14 ]]
        dof[i] <- length(fit2$coefficients)
      }
      n.tests[vim + 1] <- length( ind[-sela] )
      stat <- (devi1 - devi2) / (dof - d1) / phi
      pval <- pf(stat, dof - d1, n - dof, lower.tail = FALSE, log.p = TRUE)
      s <- which(pval < sig)
      sel <- which.min(pval) * ( length(s)>0 )
      sa <- c(sa, stat[sel]) 
      pva <- c(pva, pval[sel])
      sela <- c(sela, sel[sel>0])
      s <- s[ - which(s == sel) ]
      if (sel > 0) {
        devi1 <- devi2[sel] 
        d1 <- dof[sel]
        devi2 <- rep(devi1, p)
        dof <- phi <- rep(10000, p)
      }    
      while ( sum(s > 0) > 0 ) {
        for ( i in ind[s] )  {
          fit2 <- glm( y ~., data = x[, c(sela, i)], family = oiko, weights = wei, model = FALSE )
          devi2[i] <- fit2$deviance
          phi[i] <- summary(fit2)[[ 14 ]]
          dof[i] <- length(fit2$coefficients)
        }
        n.tests[vim + 1] <- n.tests[vim + 1] + length( ind[s] )
        stat <- (devi1 - devi2) / (dof - d1) / phi
        pval <- pf(stat, dof - d1, n - dof, lower.tail = FALSE, log.p = TRUE)
        s <- which(pval < sig)
        sel <- which.min(pval) * ( length(s)>0 )
        sa <- c(sa, stat[sel]) 
        pva <- c(pva, pval[sel])
        sela <- c(sela, sel[sel>0])
        s <- s[ - which(s == sel) ]
        if (sel > 0) {
          devi1 <- devi2[sel] 
          d1 <- dof[sel]
          devi2 <- rep(devi1, p)
          dof <- phi <- rep(10000, p)
        }  
      } ## end while ( sum(s > 0) > 0 ) 
       card <- c(card, sum(sela>0) )
    }  ## end while ( vim < K )
  } ## end if ( K > 1)
  } ## end if ( length(s) > 0 )
  
  runtime <- proc.time() - runtime
  len <- sum( sela > 0 )
  if (len > 0) {
    res <- cbind(sela[1:len], sa[1:len], pva[1:len] )
    info <- matrix(nrow = length(card), ncol = 2)
    info[, 1] <- card
    info[, 2] <- n.tests
  } else {
    res <- matrix(c(0, 0, 0), ncol = 3)
    info <- matrix(c(0, p), ncol = 2)
  }  
  colnames(res) <- c("Vars", "stat", "log p-value")
  rownames(info) <- paste("K=", 1:length(card)- 1, sep = "")
  colnames(info) <- c("Number of vars", "Number of tests")
  list(univ = univ, res = res, info = info, runtime = runtime)
}
