testIndGEEReg = function(target, reps = NULL, group, dataset, xIndex, csIndex,  wei =  NULL, univariateModels = NULL,
  ...
  if (length(cs) == 0)  {
    ...
    if ( is.null(reps) ) {
      ...
      fit2 <- try( geepack::geeglm( target ~ reps + dataset[, xIndex], family = gaussian, id = group, weights = wei, corstr = correl, std.err = se ), silent = TRUE) 
    }
    ...
    if ( is.null(reps) ) {
      ...
      fit2 <- try( geepack::geeglm( target ~ reps + dataset[, csIndex] + dataset[, xIndex], family = gaussian, id = group, weights = wei, corstr = correl, std.err = se ), silent = TRUE) 
    }  
  }
