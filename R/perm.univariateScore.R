perm.univariateScore = function(target, dataset, test, wei = NULL, dataInfo = NULL, targetID, robust, threshold, R, ncores) {
  #how many tests
  ...
  for ( i in 1:nTests ) {
    ...
    if ( i != targetID ) {
      test_results = test(target, dataset, xIndex=i, csIndex = 0, wei = wei, robust = robust, threshold = threshold, R = R)
