internaliamb.tobitbs <- function(target, dataset, threshold, wei, p, heavy = FALSE) {
  if ( !is.null(dataset) |  p > 0 ) {
    if ( p > 1 ) {
        ma <- survival::survreg( target ~.,  data = data.frame(dataset), weights = wei, dist = "gaussian" )
        ini <- 2 * logLik(ma)
        dofini <- length( coef(ma) )
        stat <- dof <- numeric(p)
        for (i in 1:p) {
          mod <- survival::survreg( target ~.,  data = data.frame(dataset[, -i]), dist = "gaussian" )
          stat[i] <- ini - 2 * logLik(ma)
          dof[i] <- dofini - length( coef(mod) ) 
        }
      
    } else {
      ini <- survival::survreg( target ~.,  data = data.frame(dataset), weights = wei, dist = "gaussian" )
      mod0 <- survival::survreg( target ~ 1,  data = data.frame(dataset), weights = wei, dist = "gaussian" )
    ...
    if ( length(sel) == 0 ) {
      ...
      if ( p - length(sel) == 0 ) {
        ...
        mod1 <- survival::survreg( target ~ .,  data = data.frame(dat), weights = wei, dist = "gaussian" )
        mod0 <- survival::survreg( target ~ 1,  data = data.frame(dat), weights = wei, dist = "gaussian" )
      ...
      } else  final <- survival::survreg( target ~ .,  data = data.frame(dat), weights = wei, dist = "gaussian" )
    }
