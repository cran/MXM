...
cv.waldmmpc <- function(target, dataset, wei = NULL, kfolds = 10, folds = NULL, alphas = c(0.1, 0.05, 0.01), max_ks = c(3, 2), task = NULL, metric = NULL, modeler = NULL, mmpc_test = NULL, ncores = 1) 
{
  if ( ncores > 1 ) {  ## multi-threaded task
    result = cvwaldmmpc.par(target, dataset, wei = wei, kfolds = kfolds, folds = folds, alphas = alphas, max_ks = max_ks, task = task, metric = metric, modeler = modeler, mmpc_test = mmpc_test, ncores = ncores)
    ...
    for (k in 1:kfolds) {
      ...
      for (mmpc_conf_id in 1:nmmpcConfs) {
        ...
        #running mmpc
        results <- wald.mmpc(train_target, train_set, max_k, threshold, test = test, ini = mmpcini, wei = wtrain, hash = TRUE, hashObject = mmpcHashMap)
