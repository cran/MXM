ebic.glm.bsreg <- function(target, dataset, wei = NULL, gam = NULL, type = "logistic") {
  if (type == "logistic") {
    oiko <- binomial(logit)
  ...
  if ( p > n ) {
    ...
    if ( M == 0 ) {
      mod <- glm( target ~ 1, family = oiko, weights = wei)
      bic <- BIC(mod)      
      if (bic0 - bic < 0 ) {
    ...
    ###########   
    for (j in 1:p) {
      mod <- glm( target ~.,  data = dataset[, -j, drop = FALSE], family = oiko, weights = wei)
    ...
    if ( bic0 - mat[sel, 2] < 0  ) {
      ...
      if ( tool[2] > 0 ) {
        ...
        while ( tool[i - 1] - tool[i ] > 0  &  NCOL(dat) > 0 )  {   
          ...
          ini <- glm( target ~., data = dat, family = oiko, weights = wei )
          M <- dim(dat)[2]
          ...
          if ( M == 1 ) {
            ...
            M <- dim(dat)[2] - 1
            for ( j in 1:(M + 1) ) {
              mod <- glm( target ~.,  data = dat[, -j, drop = FALSE], family = oiko, weights = wei )
              bic[j] <-  BIC(mod) + con * lchoose(p, M)
            }
            mat[, 2] <- bic
            sel <- which.min( mat[, 2] )
