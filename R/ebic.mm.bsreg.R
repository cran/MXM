ebic.mm.bsreg <- function(target, dataset, wei = NULL, gam = NULL) {
  ...
  if ( p > n ) {
    ...
    ini <- MASS::rlm( target ~.,  data = dataset, weights = wei, method = "MM", maxit = 2000 )
    bic0 <-  BIC(ini)   ## initial bic  
    tool[1] <- bic0
    bic <- numeric(p)
    M <- dim(dataset)[2] - 1
    if ( M == 0 ) {
      mod <- MASS::rlm( target ~ 1, weights = wei)
      bic <- BIC(mod)      
      if (bic0 - bic < 0 ) {
      ...
      ###########   
    for (j in 1:p) {
      mod <- MASS::rlm( target ~.,  data = dataset[, -j, drop = FALSE], weights = wei, method = "MM", maxit = 2000 )
    ...
    if ( bic0 - mat[sel, 2] < 0  ) {
      ...
      if ( tool[2] > 0 ) {
        ...
        while ( tool[i - 1] - tool[i ] > 0  &  NCOL(dat) > 0 )  {   
          ...
          ini <- MASS::rlm( target ~., data = dat, weights = wei, method = "MM", maxit = 2000 )
          M <- dim(dat)[2]
          bic0 <-  BIC(mod) + con * lchoose(p, M)
          i <- i + 1        

          if ( M == 1 ) {
            mod <- MASS::rlm(target ~ 1, weights = wei, method = "MM", maxit = 2000 )
            bic <-  BIC(mod)
            tool[i] <- bic
            if (bic0 - bic < 0 ) {
              runtime <- proc.time() - tic
            ...
            M <- dim(dat)[2] - 1
            for ( j in 1:(M + 1) ) {
              mod <- MASS::rlm( target ~.,  data = dat[, -j, drop = FALSE], weights = wei, method = "MM", maxit = 2000 )
              bic[j] <-  BIC(mod) + con * lchoose(p, M)
            }
            mat[, 2] <- bic
            sel <- which.min( mat[, 2] )
